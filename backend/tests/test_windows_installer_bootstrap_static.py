...
class WindowsInstallerBootstrapStaticTests(unittest.TestCase):
    ...
    def test_build_windows_packages_first_run_launcher_scripts(self):
        ...
        self.assertIn('Copy-Item -Path (Join-Path $ROOT "Launch-Sentinel-Pulse.bat")', text)
        self.assertIn('Copy-Item -Path (Join-Path $ROOT "Launch-Sentinel-Pulse.ps1")', text)
        self.assertIn('Copy-Item -Path (Join-Path $ROOT "Setup-And-Launch.bat")', text)
